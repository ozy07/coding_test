...
public class Hourly implements Employee {
    ...
    public Hourly(int id, String name) {
        this.id = id;
        this.name = name;
        this.workDaysPerYear = 260;
        this.vacationDaysAvailable = 0;
    }
    ...
    private void updateEarnedVacation(int daysWorked) {
        float workDaysForOneVactionDay = (float) 26; // 260 / 10
        vacationDaysAvailable += Precision.round(daysWorked / workDaysForOneVactionDay, 1); // single floating point
    }
}
